    ...
    <div class="main-content">
        <div class="main-content-inner">
            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        <form class="form-inline col-md-8" style="margin: 10px" action="${pageContext.request.contextPath}/FindByPageServlet?goTo=viewPage.jsp&type=View" method="post">
                            ...
                            <div class="form-group col-md-5">
                                ...
                                <input type="text" class="form-control" id="name" placeholder="风景名" name="name" value="${condition.name[0]}">
                            </div>
                            <button type="submit" class="btn btn-default">查询</button>
                        </form>

                        <a class="btn btn-primary col-md-1 pull-right" style="margin-right: 5px" href="${pageContext.request.contextPath}/back/viewForm.jsp">添加</a>
                        <a class="btn btn-primary col-md-1 pull-right" id="delSelected">删除选中</a>

                        <form id="form" action="${pageContext.request.contextPath}/DelSelectedServlet?type=View" method="post">
                            <table class="table table-hover col-xs-12 " id="grid-table"
                                ...
                                <c:forEach items="${pageBean.list}" var="view">
                                    <tr>
                                        ...
                                        <td>
                                            <a class="ace-icon fa fa-pencil  light-blue bigger-110" title="修改" href="${pageContext.request.contextPath}/FindOnlyServlet?id=${view.id}&type=View"></a>
                        ...
                        <nav class="col-md-5" aria-label="Page navigation">
                            <ul class="pagination">
                                ...
                                <c:if test="${pageBean.currentPage != 1}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/FindByPageServlet?currentPage=${pageBean.currentPage-1}&rows=5&goTo=viewPage.jsp&type=View&id=${condition.id[0]}&name=${condition.name[0]}"
                                           aria-label="Previous">
                                            <span aria-hidden="true">&laquo;</span>
                                        </a>
                                    </li>
                                ...
                                <c:forEach begin="1" end="${pageBean.totalPage}" var="i">
                                    <c:if test="${pageBean.currentPage==i}">
                                        <li class="active"><a href="${pageContext.request.contextPath}/FindByPageServlet?currentPage=${i}&rows=5&goTo=viewPage.jsp&type=View&id=${condition.id[0]}&name=${condition.name[0]}">${i}</a>
                                        </li>
                                    </c:if>
                                    <c:if test="${pageBean.currentPage!=i}">
                                        <li>
                                            <a href="${pageContext.request.contextPath}/FindByPageServlet?currentPage=${i}&rows=5&goTo=viewPage.jsp&type=View&id=${condition.id[0]}&name=${condition.name[0]}">${i}</a>
                                        </li>
                                    </c:if>
                                </c:forEach>
...
<script>
    ...
    function deleteOnly(id){
        if (confirm("确定删除吗?")){
            location.href = "${pageContext.request.contextPath}/DelSelectedServlet?uid="+id+"&type=View";
        }
    }
